...
<Tabs items={['Preview', 'Code']}>
  <Tab value="preview">
  <ComponentSource name="linear-card-demo" />
  </Tab>
  <Tab value="code">
    ```tsx
'use client'
...
export function LinearCardDemo() {
  const [selectedOffering, setSelectedOffering] =
    useState<LinearCardProps | null>(null)
...
function Card(props: { offering: LinearCardProps; onClick: () => void }) {
return (
